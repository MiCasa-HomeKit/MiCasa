...
class BridgeDelegate: DeviceDelegate {
  ...
  func didRequestIdentificationOf(_ accessory: Accessory) {
    ...
    do {
      try Bridge.shared.accessoryMap[accessory]?.identify(accessory: accessory)
    } catch {
      Bridge
        .shared
        .logger
        .error("Error while trying to request identificytion of accessory \(accessory): \(error)")
    }
  }
  ...
  func characteristic<T>(
    ...
    Bridge
      .shared
      .logger
      .info(.init(stringLiteral: "Characteristic \(characteristic) in service \(service.type) of "
        + "accessory \(accessory.info.name.value ?? "") "
    ...
    do {
      ...
      Bridge
        .shared
        .logger
        .error("Error while trying to send characteristics of accessory \(accessory): \(error)")
    }
  }
  ...
  func characteristicListenerDidSubscribe(
    ...
    Bridge
      ...
      .logger
      .info(.init(stringLiteral: "Characteristic \(characteristic) in service \(service.type) of "
        + "accessory \(accessory.info.name.value ?? "") got a subscriber"))
  }
  ...
  func characteristicListenerDidUnsubscribe(
    ...
    Bridge
      ...
      .logger
      .info(.init(stringLiteral: "Characteristic \(characteristic) in service \(service.type) of "
        + "accessory \(accessory.info.name.value ?? "") lost a subscriber"))
  }
